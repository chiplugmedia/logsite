        ...
        if ($numrow_products == 0) {
            ...
            if ($totalPrice > $funds) {
                ...
                for ($i = 1; $i <= $quantity; $i++) {
                    $purchaseReference = get_rand_alphanumeric(30); // Generate a unique reference for each purchase
                    $sql = $link->prepare("INSERT INTO userpurchases(username, title, purchaseinfo, description, amount, status, reference, date) VALUES(?, ?, ?, ?, ?, ?, ?, ?)");
                    $sql->bind_param("sssdssss", $username, $stock, $purchaseinfo, $description, $price, $status, $purchaseReference, $dateTime);
