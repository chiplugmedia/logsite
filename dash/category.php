...
// Assuming you want to fetch stocks for a particular service
$service = isset($_GET['service']) ? $_GET['service'] : ''; // Retrieve the service from the URL
$stocks = listStocks($service); // Pass the service parameter to the function

// Add 2500 to each stock's cost
                                                    ...
                                                    <li class="list-group-item px-0 py-2">
                                                        <div class="d-flex align-items-center">
                                                            <div class="flex-shrink-0">
                                                                <img src="<?php echo htmlspecialchars($stock['icon'] ?? ''); ?>" alt="img" class="wid-35 rounded-circle">
                                                            </div>
                                                            <div class="flex-grow-1 mx-3">
                                                                <p class="mb-1">
                                                                    <i class="text-small text-dark" style="font-style: normal; font-size: 12px;">
                                                                                <?php echo htmlspecialchars($stock['year'] ?? ''); ?>
                                                                            </i>
                                                                        </button>
                                                                    </form>
                                                                </p>
                                                                <p class="mb-0 text-muted">
                                                                    <span class="text-white btn btn-dark btn-rounded btn-sm" style="font-size: 12px; font-weight: bolder">Price: ₦<?php echo htmlspecialchars($stock['cost'] ?? ''); ?></span>
                                                                    | <span class="text-white btn btn-dark btn-rounded btn-sm" style="font-size: 12px; font-weight: bolder"><?php echo htmlspecialchars($stock['stock'] ?? ''); ?></span>
                                                                </p>
                                                            </div>
                                                            <div class="col-2">
                                                                <div class="d-flex justify-content-center row">
                                                                    ...
                                                                    <div class="col-12 d-flex justify-content-center">
                                                                        <?php 
                                                                            $stock_quantity = intval($stock['stock'] ?? 0);
                                                                            if ($stock_quantity > 0): 
                                                                        ?>
                                                                            <i class="btn btn-sm" style="font-size: 8px;">
                                                                                <form action="../dash/p2preg.php" method="GET">
                                                                                    <input type="hidden" name="service" value="<?php echo htmlspecialchars($stock['service'] ?? ''); ?>">
                                                                                    <input type="hidden" name="year" value="<?php echo htmlspecialchars($stock['year'] ?? ''); ?>">
                                                                                    <input type="hidden" name="cost" value="<?php echo htmlspecialchars($stock['cost'] ?? ''); ?>">
                                                                                    <input type="hidden" name="description" value="<?php echo htmlspecialchars($stock['description'] ?? ''); ?>">
                                                                                    <input type="hidden" name="icon" value="<?php echo htmlspecialchars($stock['icon'] ?? ''); ?>">
                                                                                    <input type="hidden" name="stock" value="<?php echo htmlspecialchars($stock['stock'] ?? ''); ?>">
