...
if(isset($_POST['siteDetails'])){
    ...
    $enableAct = $enableRef = $enableSpin = $enableCameroon = $enableNigeria = $enableGhana = $enableSierraLeone = $enableSouthAfrica = $enableKenya = $enableTanzania = $enableUganda = $enableAmerica = 0;
    ...
    if ($numrow == 0) {
        $sql = $link->prepare("INSERT INTO sitedetails (sitename, sitedesc, sitetag, apikey, refwithdraw, mainwithdraw, spinwheel, CameroonWithdraw, NigeriaWithdraw, GhanaWithdraw, SierraLeoneWithdraw, SouthAfricaWithdraw, KenyaWithdraw, TanzaniaWithdraw, UgandaWithdraw, AmericaWithdraw, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    } else {
        $sql = $link->prepare("UPDATE sitedetails SET sitename=?, sitedesc=?, sitetag=?, apikey=?, refwithdraw=?, mainwithdraw=?, spinwheel=?, CameroonWithdraw=?, NigeriaWithdraw=?, GhanaWithdraw=?, SierraLeoneWithdraw=?, SouthAfricaWithdraw=?, KenyaWithdraw=?, TanzaniaWithdraw=?, UgandaWithdraw=?, AmericaWithdraw=?, date=?");
    }

    $sql->bind_param("ssssiiiiiiiiiiiis", $sitename, $siteDesc, $sitetag, $apiKey, $enableRef, $enableAct, $enableSpin, $enableCameroon, $enableNigeria, $enableGhana, $enableSierraLeone, $enableSouthAfrica, $enableKenya, $enableTanzania, $enableUganda, $enableAmerica, $date);

    if ($sql->execute()) {
        // Assigning values to variables after successful execution
...
if(isset($_POST['saveFlutterwave'])){
    if(empty($_POST['flwSecretKey'])){
        $status="error";
        $message="Enter flutterwave secret key";
        $genMsg=sendResponse($status, $message);
    }
    else if(empty($_POST['flwPublicKey'])){
        $status="error";
        $message="Enter flutterwave public key";
        $genMsg=sendResponse($status, $message);
    }
    else{
        ...
        $numrow=$result->num_rows;
        if($numrow == 0){
            $sql=$link->prepare("INSERT INTO sitedetails(flwsecretkey, flwpublickey, date) VALUES(?,?,?)");
        }
        else{
            $sql=$link->prepare("UPDATE sitedetails SET flwsecretkey=?, flwpublickey=?, date=?");
        }
        $sql->bind_param("sss", $flwSecretKey, $flwPublicKey, $dateTime);
        if($sql->execute()){
            $status="success";
            $message="Flutterwave keys updated";
            $genMsg=sendResponse($status, $message);
        }
        else{
            $status="error";
